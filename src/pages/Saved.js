...
const SaveBtn = ({ data }) => {
  ...
  return (
    <button onClick={e => handleClick(e)} className=" outline-0 border-0 bg-none cursor-pointer">
      <svg className={`w-[1.5rem] ml-1.5 hover:fill-cyan ${allCoins.includes(data.id) ? "fill-cyan" : "fill-gray-100"}`} width="30" height="30" viewBox="0 0 30 30" fill="cyan" xmlns="http://www.w3.org/2000/svg">
        <g>
    ...
    <section className="w-[80%] h-full flex flex-col mt-16 mb-24 relative ">
      <div className="w-full min-h-[60vh] py-8 border border-gray-100 rounded">
        {savedData ? (
              ...
              {savedData.map(data => {
                return (
                  <tr key={data.id} className=" text-center text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0">
                    ...
                    {/**showing price in USD with the help of INTL library */}
                    <td className=" py-4">
          ...
          <h1 className=" min-h-[60vh] text-lg text-cyan flex items-center justify-center">There is no data to display.</h1>
        )}

        <button onClick={resetSavedResult} className="w-[2rem] ml-4 hover:scale-110 transition-all ease-linear absolute right-0 -top-10 ">
          <svg
