              ...
              {cryptoData.map(data => {
                return (
                  <tr key={data.id} className=" text-center text-base border-b border-gray-100 hover:bg-gray-200 last:border-b-0">
                    ...
                    <td className={data.price_change_percentage_7s_in_currency > 0 ? "text-green py-4" : "text-red py-4"}>{Number(data.price_change_percentage_7d_in_currency).toFixed(2)}</td>
                  </tr>
                )
              })}
            </tbody>
